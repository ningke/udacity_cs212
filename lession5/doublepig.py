...
def strategy_d(state):
    ...
    (p, me, you, pending, double) = state
    if double == 'double':
        return 'decline' if (you - me) > 40 else 'accept'
    elif double == 1 and (me + pending - you) > (goal - me - pending):
        #print "double!", state
        global doubled
        doubled += 1
        return 'double'
...
def test():
    ...
    res = strategy_compare(strategy_d, hold_20_d, 1000) > 60 # must win 60% of the points      
