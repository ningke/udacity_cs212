...
def more_pour_problem(capacities, goal, start=None):
    ...
    succ = lambda st: successors(st, capacities)
    return shortest_path_search(start, succ, goal_func(goal))

def goal_func(goalamount):
    def is_goal(state):
        return any(c == goalamount for c in state)
    return is_goal
...
def successors(state, capacities):
    ...
    for idx in rg:
        cap = capacities[idx]
        # fill the glass
        succ[state[:idx] + (cap,) + state[idx+1:]] = ('fill', idx)
        # empty the glass
        succ[state[:idx] + (0,) + state[idx+1:]] = ('empty', idx)
        # pour from idx to another glass
        if state[idx]:
            for i in rg:
                if i != idx and state[i] != capacities[i]:
                    # we either empty source or fill destination
                    room = capacities[i] - state[i]
                    if state[idx] <= room:
                        changed = {idx: 0, i: state[i] + state[idx]}
                    else:
                        changed = {idx: state[idx] - room, i: capacities[i]}
                    s1 = tuple(changed[j] if j in changed else state[j] \
                                   for j in rg)
                    succ[s1] = ('pour', idx, i)
    return succ
