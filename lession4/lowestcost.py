...
def lowest_cost_search(start, successors, is_goal, action_cost):
    ...
    while frontier:
        path = frontier.pop(0)
        cost, state = path[-1], path[-2]
        explored.add(state)
        if is_goal(state):
            return path[:-1]
        for st, act in successors(state).items():
            #print ">>>>" , st
            #print ">>>>", act
            if st not in explored:
                frontier.append(path[:-1] + [act, st, cost + action_cost(act)])
        frontier.sort(key=lambda p: p[-1])
    return False

def solve(here):
    start = tuple([frozenset(here) | frozenset(['light']), frozenset()])
    is_goal = lambda s: not s[0] or (len(s[0])==1 and 'light' in s[0])
...
print bridge_problem2({1, 2, 5, 10, 'light'})
print solve({1, 2, 5, 10, 'light'})
