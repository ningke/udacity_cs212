...
def bsuccessors2(state):
    ...
    # your code here
    here, there = state
    t, s = sorted(state, key=lambda x: 'light' in x)
    arrow = '->' if s is here else '<-'
    return dict(((s  - frozenset([a,b, 'light']),
                      t | frozenset([a, b, 'light'])),
                     (a, b, arrow))
                    for a in s if a is not 'light'
                    for b in s if b is not 'light')
