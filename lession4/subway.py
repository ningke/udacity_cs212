...
def subway(**lines):
    ...
    for linename, stations in lines.items():
        ...
        for i, st in enumerate(stlist):
            neilist = [stlist[j] for j in (i - 1, i + 1) if j >= 0 and j < stnr]
            if st not in res:
                res[st] = {}
            res[st].update(zip(neilist, [linename] * len(neilist)))
    return res
...
def ride(here, there, system=boston):
    ...
    ## your code here
    arrived = lambda s: s == there
    successors = lambda s: system[s]
    return shortest_path_search(here, successors, arrived)

def longest_ride(system):
    """"Return the longest possible 'shortest path' 
    ride between any two stops in the system."""
    ## your code here
    return max((longest_from(sta, system) for sta in system), key=len)

def longest_from(sta, system):
    seen = set()
    def _longest(curr, path):
        if curr in seen:
            return path
        seen.add(curr)
        return max((_longest(s, path + [curr, l]) for s, l in \
                        system[curr].items()), key=len)
    return _longest(sta, [])
