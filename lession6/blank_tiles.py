...
# blank is represented by '_' in a hand, but changed to a letter on the board,
# maybe use a lower case letter.
...
def word_with_wc(hand, board_word, word):
    return ''.join(l if (is_letter(bl) and l == bl) or l in hand else '_'
                         for l, bl in zip(word, board_word))
...
def calculate_score(board, pos, direction, hand, word):
    ...
    word_range = lambda x: range(x, x + len(word))
    if direction == ACROSS:
        board_word = [board[startj][i] for i in word_range(starti)]
    else:
        board_word = [board[j][starti] for j in word_range(startj)]
    wc_word = word_with_wc(hand, board_word, word)
    #print wc_word
    for (n, L) in enumerate(wc_word):
...
def horizontal_plays(hand, board):
    ...
    results = set()
    wc_hands = hands_with_wc(hand)
    print wc_hands
    for (j, row) in enumerate(board[1:-1], 1):
        set_anchors(row, j, board)
        for hd in wc_hands:
            for (i, word) in row_plays(hd, row):
...
def test():
    ...
    assert ok('KICEHKN', 61, (9, 1), (1, 0), 'KiCk')
