...
def termstr(c, i):
    ''' String representation of a polynomial term (c * x**i). '''
    xterm = lambda n: '1' if n == 0 else "x" if n == 1 else "x**%d" % i
    xt = xterm(i)
    return '' if c == 0 else \
        ("%r" % c) if i == 0 else \
        xt if c == 1 else \
...
def polyname(coefs):
    name1 = ' + '.join(filter(lambda s: s != '',
                              reversed(list(termstr(c, i)
...
def poly(coefs):
    ...
    polyfunc.coefs = tuple(coefs)
    polyfunc.__name__ = polyname(coefs)
    return polyfunc
...
def is_poly(x):
    ...
    return callable(x) \
        and hasattr(x, 'coefs') and isinstance(x.coefs, tuple)

def add(p1, p2):
    "Return a new polynomial which is the sum of polynomials p1 and p2."
    coefs1, coefs2 = p1.coefs, p2.coefs
    return poly(tuple(c1+c2 for c1, c2 in
                      itertools.izip_longest(coefs1, coefs2, fillvalue=0)))

def sub(p1, p2):
    "Return a new polynomial which is the difference of polynomials p1 and p2."
    coefs2 = tuple(-c for c in p2.coefs)
    return add(p1, poly(coefs2))

def mul(p1, p2):
    "Return a new polynomial which is the product of polynomials p1 and p2."
    coefs1, coefs2 = p1.coefs, p2.coefs
    result = list(itertools.repeat(0, len(coefs1) - 1 + len(coefs2) - 1 + 1))
    for i, c1 in enumerate(coefs1):
        for j, c2 in enumerate(coefs2):
            result[i+j] += c1 * c2
    return poly(result)
...
def power(p, n):
    ...
    if n == 0:
        ...
        m = n / 2
        return mul(power(p, m), power(p, m))
    else:
...
def deriv(p):
    ...
    return poly([c*(i+1) for i, c in enumerate(p.coefs[1:])])

def integral(p, C=0):
    "Return the integral of a function p (with respect to its argument)."
    return poly([C] + [c/(i+1) for i, c in enumerate(p.coefs)])
