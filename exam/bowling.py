...
def bowling(balls):
    ...
    ## bowling([int, ...]) -> int
    ## your code here
    score = bowling_score(1, balls, 0)
    #print "%s ==> %d" % (balls, score)
    return score

def bowling_score(frame_num, balls, score):
    if frame_num == 11:
        return score
    first, second = balls[:2]
    rest = balls[2:]
    if first == 10: # strike: only one ball
        second = 0
        rest = balls[1:]
    curr_score = frame_score(first, second, rest)
    return bowling_score(frame_num + 1, rest, score + curr_score)

def frame_score(first, second, rest):
    ''' Calculates score for a frame. '''
    if first == 10:
        # strike: add next two balls
        return 10 + sum(rest[:2])
    elif first + second == 10:
        # spare: add the next ball
        return 10 + rest[0]
    else:
        return first + second
