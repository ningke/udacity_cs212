...
points_list = [0] + \
    sorted(list(set(
            [50, 25] + \
                ...
                [i for i in range(1,21)])), reverse=True)

def make_score_map():
    ''' Score of points. For example, 36 -> ('D18', 'T12'). '''
    ptlist = [0] + \
        [50, 25] + \
        [3 * i for i in range(1,21)] + \
        [2 * i for i in range(1,21)] + \
    ...
    scorelist = [None] + \
        ["DB", "SB"] + \
        ["T%d" % i for i in range(1,21)] + \
        ["D%d" % i for i in range(1,21)] + \
        ["S%d" % i for i in range(1,21)]
    sorted_combin = sorted(zip(ptlist, scorelist), key=lambda pair: pair[0])
    grouped = ((k, tuple(x[1] for x in g)) for k, g in itertools.groupby(
            sorted_combin, key=lambda t: t[0]))
    return {pt: score for pt, score in grouped}

score_map = make_score_map()
...
def double_out(total):
    ...
    # your code here
    #print "%d =>" % total
    pick_double = lambda choices: next((sc for sc in choices if sc.startswith('D')),
                                       choices[0])
    sortseq = {'S': 0, 'T': 1, 'D': 2}
    for darts in darts_iter(total, 3):
        doubles = [pick_double(choices) for choices in darts]
        has_double = lambda darts: any(p.startswith('D') for p in doubles)
        if has_double(doubles):
            #print doubles
            return sorted(doubles, key=lambda x: sortseq[x[0]])
    return None

def parts_of_whole(target, partial, maxnum):
    nump = len(partial)
    assert(nump <= maxnum)
    #print "%d: %s/%s" % (target, partial, [points_list[p] for p in partial])
    if target < 0:
        return set()
    if target == 0:
        return set([partial])
    ...
    choices = range(last, len(points_list))
    return reduce(lambda x,y: x | y,
                  (parts_of_whole(target - points_list[p], partial + (p,), maxnum)
                   for p in choices),
                  set())

def darts_iter(target, maxnum):
    ptlist = lambda indices: [score_map[points_list[i]] for i in indices if i != 0]
    return itertools.imap(ptlist, parts_of_whole(target, (), maxnum))
...
def pre(x):
    ''' Predecessor of x. '''
    return sections[sections.index(x) - 1]

def suc(x):
    ''' Successor of x. '''
    return sections[(sections.index(x) + 1) % len(sections)]

def tgtstr(ring, num):
    return "%s%d" % (ring, num)

def dist(dim1, dim2):
    ''' Total probability distribution given the distribution of the ring and section. '''
    def consolidate(dst):
        ''' Consolidate offboard|SB|DB distribution into one. '''
        result = {}
        #print dst
        for tgt, prob in dst.items():
            mo = re.match(r'(OFF|SB|DB)\d+', tgt)
            nm = mo.group(1) if mo else tgt
            result[nm] = result[nm] + prob if nm in result else prob
        return result
    return consolidate(
        {"%s%d" % (a, b): round(x*y, 5)
         for (a, x), (b, y) in itertools.product(dim1, dim2)})

def T(mr, x):
    ''' Triple ring: miss rate ''mr'', section ''x''. '''
    return dist((('T', 1-mr), ('S', mr)),
                ((x, 1-mr), (pre(x), mr/2.0), (suc(x), mr/2.0)))

def D(mr, x):
    ''' Double ring. '''
    return dist(
        (('D', 1-mr), ('S', mr/2.0), ('OFF', mr/2.0)),
        ((x, 1-mr), (x-1, mr/2.0), (x+1, mr/2.0)))

def S(mr, x):
    ''' Single ring. '''
    mr1 = mr / 5.0
    return dist(
        (('S', 1-mr1), ('D', mr1/2.0), ('T', mr1/2.0)),
        ((x, 1-mr), (x-1, mr/2.0), (x+1, mr/2.0)))

def bullseye(sb, db):
    ''' Given probabilities ''sb'' (single bull) and ''db'' (double bull), return
    probability distribution of darts. Missed darts goes evenly among the 20 sections. '''
    return {k:v for k,v in chain(
        (('DB', db), ('SB', sb)),
        zip(("S%d" % i for i in range(1,21)), repeat((1-sb-db)/20.0, 20)))}

def SB(mr, x):
    ''' Single Bull. '''
    return bullseye((1-mr)*(1-mr), mr*(1.0/4)*(1-mr))

def DB(mr, x):
    ''' Double Bull. '''
    mr1 = 3 * mr
    mr1 = mr1 if mr1 <= 1.0 else 1.0
    return bullseye(mr1*(1/3.0)*(1-mr1), (1-mr1)*(1-mr1))

def outcome(target, miss):
    "Return a probability distribution of [(target, probability)] pairs."
    assert(re.match(r'[TDS]\d+|SB|DB', target))
    ring, section = (target[0], int(target[1:])) if target[1] in '123456789' else \
                    (target, 0)
    return globals()[ring](miss, section)

def score(target):
    ''' Score of a target. '''
    def calc(tgt):
        mo = re.match(r'([TDS])(\d+)', tgt)
        ring, sect = mo.group(1), mo.group(2)
        factor, number = {r:x for r,x in zip("TDS", [3,2,1])}[ring], int(sect)
        return factor * number
    return 0 if target == 'OFF' else \
        25 if target == 'SB' else \
        50 if target == 'DB' else \
        calc(target)

def expected_score(target, miss):
    ''' Expected score. '''
    return sum(prob * score(tgt) for tgt, prob in outcome(target, miss).items())

def best_target(miss):
    "Return the target that maximizes the expected score."
    targets_iter = itertools.chain(('SB', 'DB'),
                                   ("%s%d" % (r,s) for r,s in itertools.product(
                                       list("TDS"), xrange(1,21))))
    return max(((tgt, expected_score(tgt, miss)) for tgt in targets_iter),
               key=lambda x: x[1])[0]
