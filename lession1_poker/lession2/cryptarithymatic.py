...
def compile_formula(formula, verbose=False):
    ...
    parms = ', '.join(letters)
    raw_tokens = re.split('([A-Z]+)', formula)
    leading_digits = set(t[0] for t in raw_tokens if t.isupper() and len(t) > 1)
    tokens = map(compile_word, raw_tokens)
    body = ''.join(tokens)
    body2 = 'all(i > 0 for i in [%s])' % ', '.join(leading_digits)
    f = 'lambda %s: %s and %s' % (parms, body2, body)
    if verbose: print f
